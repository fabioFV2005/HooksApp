...
export const InstagromApp = () => {
    ...
    const handleAddComment = async (formData: FormData) => {
        ...
        startTransition(async () => {
            // simulate request http to the server
            ...
            //         text: messageText,
            //     }])
            //! code to revert the optimistic update in case of error
            setComments(prev => prev);
            toast('error sending comment', {
                description: 'there was an error sending your comment',
                duration: 10_000,
                position: 'top-right',
                action: {
                    label: 'Close',
        ...
        <div className="bg-slate-700 h-screen flex flex-col items-center justify-center">
            ...
            >
                ...
                <button
                    type="submit"
                    disabled={isPending}
                    className="bg-blue-500 text-white p-2 rounded-md w-full disabled:opacity-50 cursor-pointer"
                >
                    Enviar
                </button>
            </form>
